...
class TugasPeletonController extends Controller
{
    ...
    public function create()
    {
        ...
        $siswa = Siswa::whereNotIn('id', $siswaAktifIds)->get();
        $tugasSiswaStatus = TugasSiswa::whereIn('siswa_id', $siswa->pluck('id'))
                            ->pluck('status', 'siswa_id')->toArray();
        return view('superadmin.tugaspeleton.create', compact('pengasuh', 'user', 'siswa', 'tugasSiswaStatus', 'siswaAktifIds'));
    }
    ...
    public function store(Request $request)
    {
        ...
        foreach ($validated['siswa_id'] as $siswaId) {
            $existing = TugasSiswa::where('siswa_id', $siswaId)->first();
